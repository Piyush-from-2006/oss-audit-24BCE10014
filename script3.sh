...
# Iterating through each path defined in the DIRS array
for DIR in "${DIRS[@]}"; do
    # Checking if the directory actually exists on this Ubuntu system
    if [ -d "$DIR" ]; then
        # Using awk to grab Permissions (1), Owner (3), and Group (4)
        PERMS=$(ls -ld "$DIR" | awk '{print $1, $3, $4}')
        # Using du and cut to get only the human-readable size
...
echo " "
# --- Software Specific Config Audit ---
